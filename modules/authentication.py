...
def existing_user(username):
    user = Users.gql("WHERE u_name = :name", name = username)
...
class SignUp(AppHandler):
    ...
    def post(self):
        ...
        if error:
            ...
            u = Users(u_name = username, 
                      p_hash = make_phash(username, password), 
                      email = email)
            user = u.put()
...
class Login(AppHandler):
    ...
    def post(self):
        ...
        user = Users.all().filter('u_name =', username).get()
        if user:
            p_hash = user.p_hash
        
        if user and validate_password(username, password, p_hash):
